  ...
  <div class="row">
    <div class="mchat_container" @click="readmessage()">
      ...
      <div
        class="chat_body"
        ref="chatbody"
        v-chat-scroll="{ always: false, smooth: false, image: true }"
        style="overflow-y: auto"
        @scroll="scrollReached()"
      >
      ...
      <span class="btn-scroll-bottom" v-if="show_scroll_btn" @click="scrollToBottom()"
        ><img src="/imgs/down_arrow.png" width="40" alt=""
      /></span>
...
export default {
      ...
      if (chatmessage.file) {
        ...
        this.axios.post(url, params).then((response) => {
          if (response.status) {
            ...
            firebase
              .database()
              .ref('chatrooms/' + this.receiver + '/read/' + this.sender)
              .update({ read: false })
          }
        })
      }
  ...
  mounted() {
    ...
    if (!this.$route.params.userdetail.url) {
      this.logo_url = this.$route.params.userdetail.profile_pic
        ? this.$route.params.userdetail.profile_pic.url
        : '/imgs/default_sm.png'
    }
    this.fetchMessages()
    let _this = this
    ...
    firebase
      .database()
      .ref('chatrooms/' + loged_user + '/chats/')
      .remove()
    // Sent
    let starCountRef = firebase
      .database()
      .ref('chatrooms/' + _this.id + '/chats/')
      .limitToLast(1)
    ...
    // Received
    let starCountRef1 = firebase
      .database()
      .ref('chatrooms/' + _this.sender + '/chats/')
      .limitToLast(1)
  ...
  created() {
    if (!this.$route.params.userdetail) {
      this.$router.push('/mobile/chatlist')
    } else {
      this.receiver = this.$route.params.userdetail.id
      this.id = this.$route.params.userdetail.id
      this.name = this.$route.params.userdetail.name
      this.username = this.$route.params.userdetail.username
      this.logo_url = this.$route.params.userdetail.url
