<template>
  <div class="row" id="comment_container">
    ...
    <div class="comment_body pt-2" v-if="media">
      <div class="comment_header" v-if="media.description">
        <div class="media_user my-0 px-2 clearfix" v-if="media.portal">
          <img
            :src="serverUrl(media.portal.media.url)"
            v-if="media.portal.media"
            alt
            @click="goProfile(media.portal.name)"
          />
          ...
          <div class="username">
            ...
            <div class="user_icons">
              <img
                ...
                v-if="media.portal.store_type === 2 || media.portal.store_type === 3"
                alt
              />
              <img
                src="~assets/imgs/dispensary.png"
                v-if="media.portal.store_type === 1 || media.portal.store_type === 3"
                alt
              />
            </div>
          </div>
        ...
        <div class="media_user my-0 px-2 clearfix" v-else>
          <img
            :src="serverUrl(media.user.profile_pic.url)"
            v-if="media.user.profile_pic"
            alt
            @click="goProfile(media.user.name)"
          />
      ...
      <div class="comment_container" v-for="(item, index) in comments" v-if="!loading" :key="index">
        <div class="comment">
          ...
          <div class="description">
            <a v-bind:href="'/' + item.user.name"
              ><span class="username">{{ item.user.name }}</span></a
            >
          ...
          <div class="sub2_comment_container" v-for="(sub2_item, sub2index) of sub_item.sub2_comments" :key="sub2index">
            <div class="sub2_comment">
              ...
              <div class="description">
                ...
                <p class="status">
                  <span class="likes" v-if="sub2_item.likes > 0 && sub2_item.likes == 1"
                    >{{ sub2_item.likes }} Like.</span
                  >
                  <span class="likes" v-if="sub2_item.likes > 0 && sub2_item.likes > 1"
                    >{{ sub2_item.likes }} Likes.</span
                  >
                  <!-- <span class="reply" @click="replyto(sub_item.id, sub_item.user.name)">Reply</span> -->
                </p>
              </div>
              <div class="actions">
                <fa
                  icon="heart"
                  @click="unlikecomment(sub2_item.id, sub2index, subindex, index)"
                  v-if="sub2_item.userliked"
                ></fa>
    ...
    <div class="mobile_comment_box">
      ...
      <textarea
        class="comment_text_box"
        id="mobile_comment"
        v-model="commenttext"
        onfocus="this.value = this.value;"
      ></textarea>
      <a href="javascript:;" id="btn_post" @click="postcomment" v-if="!editComment"
          ...
          this.axios.post(uri, params).then((response) => {
            ...
            if (this.user.id != this.media.user_id) {
              let noti_fb = firebase
                .database()
                .ref('notifications/' + this.media.user_id)
                .push()
              if (this.replyid) {
                noti_fb.set({
      ...
      if (this.user) {
        ...
        this.axios.post(uri, params).then((response) => {
          ...
          if (this.user.id != action_comment.user_id) {
            let noti_fb = firebase
              .database()
              .ref('notifications/' + action_comment.user_id)
              .push()
            noti_fb.set({
              notifier_id: this.user.id,
              type: 'like_comment',
